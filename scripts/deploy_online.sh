...
usage() {
  if [[ "$UNIROBOGUI_LANG" == "en" ]]; then
    cat <<'EOF'
Usage:
  bash scripts/deploy_online.sh --product g1|r1
...
If the robot cannot access GitHub, run this on an Ubuntu/Linux computer that can:
  bash scripts/deploy.sh from-pc --product g1|r1 --host USER@HOST
...
while (($#)); do
  case "$1" in
    --product)
      (($# >= 2)) || { ui_err "[错误] --product 缺少 g1|r1" "[ERROR] --product requires g1|r1"; exit 2; }
      PRODUCT="$2"
      shift
      ;;
...
[[ -n "$PRODUCT" ]] || { ui_err "[错误] 必须指定 --product g1|r1" "[ERROR] --product g1|r1 is required"; exit 2; }
...
update_or_clone() {
  ...
  if [[ -e "$dir" ]]; then
    ...
    if [[ ! -d "$dir/.git" ]]; then
      ...
      if [[ "$remote" != *"$expected"* ]]; then
        ...
        help_customer "如果这里 Git 拉取失败，请检查机器人 wlan0、DNS 和 GitHub 访问；不要操作 ${DDS_INTERFACE}。若 GitHub 无法访问，请改用 deploy.sh from-pc。" "If Git pull fails, check wlan0, DNS, and GitHub access. Do not modify ${DDS_INTERFACE}; use deploy.sh from-pc when GitHub is unavailable."
  ...
  if [[ ! -e "$dir" ]]; then
    ...
    help_customer "如果 clone 失败，请检查机器人能否访问 GitHub；无法访问时请改用 deploy.sh from-pc。" "If clone fails, verify GitHub access; otherwise use deploy.sh from-pc on an online computer."
...
update_project_checkout() {
  ...
  [[ -d "$PROJECT_DIR/.git" ]] ||
    die "在线部署要求 /home/unitree/UniRoboGui 是 Git 仓库；机器人不能访问 GitHub 时请改用 deploy.sh from-pc。" "Online deployment requires a Git checkout at /home/unitree/UniRoboGui; use deploy.sh from-pc when the robot cannot access GitHub."
  local remote dirty
...
check_environment() {
  ...
  [[ "$(id -un)" == "unitree" ]] || die "当前用户是 $(id -un)，本脚本仅支持机器人 unitree 用户。" "Current user is $(id -un); this script supports only the robot's unitree account."
  [[ "$(uname -m)" == "aarch64" ]] || die "当前架构为 $(uname -m)，预期 ${PRODUCT_NAME} aarch64。" "Current architecture is $(uname -m); expected ${PRODUCT_NAME} aarch64."
  ...
  ip link show dev "$DDS_INTERFACE" >/dev/null 2>&1 || die "未发现 ${DDS_INTERFACE}。" "${DDS_INTERFACE} was not found."
  ok "系统、用户、CPU 架构、无 ROS 环境和 ${DDS_INTERFACE} 检查通过。" "System, user, CPU architecture, clean ROS environment, and ${DDS_INTERFACE} checks passed."
}
...
if $OFFLINE_MODE; then
  ...
  if github_available; then
    ...
    if command -v nmcli >/dev/null 2>&1 &&
      ...
      warn "检测到 wlan0 为 wifi:unavailable。可先检查 rfkill；不要操作 ${DDS_INTERFACE}。" "wlan0 is unavailable. Check rfkill and do not modify ${DDS_INTERFACE}."
    fi
    echo
    ui_line "原因：在线流程需要 GitHub 更新 UniRoboGui，并按所选产品准备需要的上游依赖。" "Reason: the online flow needs GitHub to update UniRoboGui and prepare upstream dependencies required by the selected product."
    ui_line "请不要在机器人上反复重试，也不要断开或修改 ${DDS_INTERFACE}。" "Do not keep retrying on the robot, and do not disconnect or modify ${DDS_INTERFACE}."
    echo
    ...
    echo "  cd UniRoboGui"
    echo "  bash scripts/deploy.sh from-pc --product ${PRODUCT_ID} --host ${DEFAULT_HOST}"
    echo
    ui_line "该脚本只准备所选产品需要的依赖，然后通过 SSH/rsync 传到机器人并自动编译安装。" "The script prepares only the dependencies required by the selected product, transfers them via SSH/rsync, and builds/installs on the robot."
    exit 20
  fi
fi

step "2/8 检测 ${PRODUCT_NAME} 环境" "2/8 Check the ${PRODUCT_NAME} environment"
check_environment
...
if ((${#missing_packages[@]} == 0)); then
  ...
  info "后续统一由 install.sh 处理：交互 sudo 模式会安装缺失包；无 sudo 的用户级模式会明确停止并提示先补包。" "install.sh handles this next: system install can add missing packages; user-local mode stops with a clear message."
fi

step "5/8 准备 Unitree SDK2 源码" "5/8 Prepare Unitree SDK2 source"
if [[ "$SDK_POLICY" == verify_existing ]]; then
  info "R1 复用 /usr/local 的现有 SDK2；本流程不会下载或覆盖它。" "R1 reuses the existing SDK2 under /usr/local; this flow will not download or replace it."
...
if ! ${NEEDS_REALSENSE}; then
  info "${PRODUCT_NAME} 不需要 librealsense2；跳过该依赖。" "${PRODUCT_NAME} does not require librealsense2; skipping it."
elif {
...
[[ "$PROJECT_DIR" == "/home/unitree/UniRoboGui" ]] ||
  die "项目必须位于 /home/unitree/UniRoboGui；当前目录为 $PROJECT_DIR。" "The project must be located at /home/unitree/UniRoboGui; current directory: $PROJECT_DIR."
[[ -f "$PROJECT_DIR/scripts/install.sh" ]] || die "缺少 scripts/install.sh。" "Missing scripts/install.sh."
if $OFFLINE_MODE && ${NEEDS_KOKORO}; then
...
if $OFFLINE_MODE; then
  UNIROBOGUI_OFFLINE=1 bash "$PROJECT_DIR/scripts/install.sh" \
    --product "$PRODUCT_ID" --sdk2-source "$SDK2_DIR" --realsense-source "$REALSENSE_DIR"
else
...
elif systemctl --user is-active --quiet "$WEB_SERVICE" 2>/dev/null; then
  service_scope=user
else
  die "system 和 user 两种 ${WEB_SERVICE} 都未处于 active 状态。" "Neither system nor user ${WEB_SERVICE} is active."
fi
...
if ${NEEDS_KOKORO} && [[ "$service_scope" == system ]] && systemctl is-enabled --quiet g1-local-tts.service 2>/dev/null; then
...
elif ${NEEDS_KOKORO} && [[ "$service_scope" == user ]] && systemctl --user is-enabled --quiet g1-local-tts.service 2>/dev/null; then
...
[[ -n "$robot_ip" ]] || robot_ip="$(ip -4 -o addr show dev "$DDS_INTERFACE" 2>/dev/null | awk '{split($4,a,"/"); print a[1]; exit}')"
echo
line
