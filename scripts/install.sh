...
source "${repo_dir}/scripts/deploy_product.sh"
...
usage() {
  if [[ "$UNIROBOGUI_LANG" == "en" ]]; then
    cat <<'EOF'
Usage: bash scripts/install.sh --product g1|r1 [options]
...
Options:
  ...
  --system-install      Force system-level systemd services (requires sudo)
  --check-only          Only check the robot, network and transferred source trees
  -h, --help            Show this help
EOF
  else
    cat <<'EOF'
用法：bash scripts/install.sh --product g1|r1 [选项]
...
选项：
  ...
  --system-install      强制系统级 systemd 服务（要求 sudo）
  --check-only          仅检查机器人、网络和已传入源码，不执行安装
  -h, --help            显示帮助
EOF
  fi
}
...
while (($#)); do
  case "$1" in
    --product)
      (($# >= 2)) || { ui_err "错误：--product 需要 g1 或 r1" "ERROR: --product requires g1 or r1"; exit 2; }
      product="$2"; shift ;;
    --sdk2-source)
...
[[ -n "${product}" ]] || { ui_err "错误：必须显式指定 --product g1|r1" "ERROR: --product g1|r1 is required"; exit 2; }
load_product_profile "${product}"
${NEEDS_REALSENSE} || without_realsense=true
${NEEDS_KOKORO} || without_kokoro=true
...
wifi_help() {
  if [[ "$UNIROBOGUI_LANG" == "en" ]]; then
    cat >&2 <<EOF
[TIP] The robot may not be connected to Wi-Fi. Keep ${DDS_INTERFACE} for robot DDS and configure wlan0 only:
  nmcli device status
  nmcli radio wifi
  rfkill list wifi
  ...
  ip -4 route show
Rerun after connecting. Do not disconnect or modify ${DDS_INTERFACE}, and do not put the Wi-Fi password on the command line.
EOF
  else
    cat >&2 <<EOF
[提示] 机器人可能尚未连接 Wi-Fi。请保留 ${DDS_INTERFACE}（机器人 DDS 专用），只配置 wlan0：
  nmcli device status
  nmcli radio wifi
  rfkill list wifi
  ...
  ip -4 route show
连接后重新运行安装脚本。不要断开或修改 ${DDS_INTERFACE}，也不要在命令中明文填写 Wi-Fi 密码。
EOF
  fi
}
...
See docs/deployment-dependencies.en.md for copy-ready offline transfer commands.
EOF
  else
    cat >&2 <<'EOF'
...
install_system_packages() {
  ...
  if ! sudo apt-get update \
    ...
    wifi_help
    fail "缺少必要软件包且 APT 更新失败。请保持 ${DDS_INTERFACE} 不变，让 wlan0 连接 Ubuntu 20.04 软件源。" "APT update failed while packages are missing. Keep ${DDS_INTERFACE} unchanged and connect wlan0 to an Ubuntu 20.04 mirror."
  fi
...
check_network() {
  if ! ip link show dev wlan0 >/dev/null 2>&1; then
    ...
    ui_err "[WARN] wlan0 当前不可用；不要操作 ${DDS_INTERFACE}。" "[WARN] wlan0 is unavailable; do not modify ${DDS_INTERFACE}."
    wifi_help
  elif ! nmcli -t -f DEVICE,TYPE,STATE device status 2>/dev/null |
         grep -q '^wlan0:wifi:connected$'; then
    ui_err "[WARN] wlan0 尚未连接 Wi-Fi；需要联网取得的软件包或产品依赖会失败。" "[WARN] wlan0 is not connected to Wi-Fi; packages or product dependencies that require Internet access will fail."
    wifi_help
    ...
    ui_line "[OK] wlan0 已连接，${DDS_INTERFACE} 保留给机器人 DDS。" "[OK] wlan0 is connected and ${DDS_INTERFACE} remains dedicated to robot DDS."
  fi
...
install_sdk2() {
  ...
  if [[ "${SDK_POLICY}" == "verify_existing" ]]; then
    [[ -f "${config}" ]] ||
      fail "R1 要求复用 ${sdk_prefix} 的现有 SDK2；未找到 ${config}，不会自动覆盖安装。" "R1 reuses the existing SDK2 under ${sdk_prefix}; ${config} is missing and will not be installed automatically."
    [[ -f "${sdk_prefix}/include/unitree/robot/r1/audio/audio_client.hpp" &&
       -f "${sdk_prefix}/include/unitree/robot/r1/loco/r1_loco_client.hpp" ]] ||
      fail "${sdk_prefix} 的 SDK2 缺少 R1 Audio/Loco 头文件；请先按机器人固件/ABI要求安装兼容 SDK。" "SDK2 under ${sdk_prefix} lacks the R1 Audio/Loco headers. Install a firmware/ABI-compatible SDK first."
    compgen -G "${sdk_prefix}/lib/libddsc.so*" >/dev/null &&
      compgen -G "${sdk_prefix}/lib/libddscxx.so*" >/dev/null ||
      fail "${sdk_prefix}/lib 缺少 R1 SDK2 所需的 CycloneDDS 动态库；不会自动覆盖 /usr/local。" "Required CycloneDDS shared libraries are missing under ${sdk_prefix}/lib; /usr/local will not be overwritten automatically."
    ui_line "[OK] 已验证并复用 R1 现有 SDK2：${sdk_prefix}" "[OK] Reusing the verified existing R1 SDK2: ${sdk_prefix}"
    return 0
  fi
  [[ ! -f "${config}" ]] || {
...
assert_running_product() {
  local manifest
  manifest="$(curl --noproxy '*' -fsS http://127.0.0.1:8080/api/robot/manifest)" ||
    fail "正在运行的 Web 服务没有返回 Manifest，因此不会替换。" "The running Web service did not return its Manifest and will not be replaced."
  MANIFEST="${manifest}" EXPECTED_PRODUCT="${PRODUCT_ID}" python3 - <<'PY'
import json
import os

actual = json.loads(os.environ["MANIFEST"]).get("identity", {}).get("product_id")
expected = os.environ["EXPECTED_PRODUCT"]
if actual != expected:
    raise SystemExit(f"[ERROR] Running Web product is {actual!r}, expected {expected!r}; refusing automatic replacement.")
PY
}

assert_no_other_product_service() {
  if systemctl is-active --quiet "${OTHER_WEB_SERVICE}" 2>/dev/null ||
     systemctl --user is-active --quiet "${OTHER_WEB_SERVICE}" 2>/dev/null; then
    fail "检测到另一产品服务 ${OTHER_WEB_SERVICE} 正在运行；不会自动停止或切换产品。" "Another product service (${OTHER_WEB_SERVICE}) is active; it will not be stopped or switched automatically."
  fi
}

verify_running_service() {
  local health manifest pid cmdline env_dump helper_rc=0

  health="$(curl --noproxy '*' -fsS http://127.0.0.1:8080/api/health)" ||
    fail "无法读取部署后的 health。" "Could not read post-deployment health."
  manifest="$(curl --noproxy '*' -fsS http://127.0.0.1:8080/api/robot/manifest)" ||
    fail "无法读取部署后的 Manifest。" "Could not read post-deployment Manifest."

  HEALTH="${health}" MANIFEST="${manifest}" EXPECTED_PRODUCT="${PRODUCT_ID}" python3 - <<'PY'
import json
import os
...
expected = os.environ["EXPECTED_PRODUCT"]
actual = manifest.get("identity", {}).get("product_id")
if actual != expected:
    raise SystemExit(f"[ERROR] Manifest product is {actual!r}, expected {expected!r}.")
if health.get("dds_initialized") is not True:
    raise SystemExit("[ERROR] DDS is not initialized.")

telemetry = next(
    (item for item in manifest.get("capabilities", []) if item.get("key") == "telemetry"),
    {},
)
required = telemetry.get("parameters", {}).get("required_sources", "")
for source in filter(None, (item.strip() for item in required.split(","))):
    if health.get("sources", {}).get(source, {}).get("status") != "online":
        raise SystemExit(f"[ERROR] Required DDS source {source!r} is not online.")
PY
  ...
  if [[ "${install_scope}" == "system" ]]; then
    ...
    pid="$(systemctl --user show "${WEB_SERVICE}" -p MainPID --value)"
  fi
  [[ "${pid}" =~ ^[1-9][0-9]*$ ]] ||
    fail "无法取得 ${WEB_SERVICE} 的 MainPID。" "Could not get MainPID for ${WEB_SERVICE}."

  cmdline="$(tr '\0' ' ' < "/proc/${pid}/cmdline")"
  [[ "${cmdline}" == *"--robot ${PRODUCT_ID}"* &&
     "${cmdline}" == *"--interface ${DDS_INTERFACE}"* ]] ||
    fail "MainPID 启动参数与目标产品不一致：${cmdline}" "MainPID arguments do not match the selected product: ${cmdline}"

  env_dump="$(tr '\0' '\n' < "/proc/${pid}/environ")"
  if grep -Eq '^(ROS_DISTRO|RMW_IMPLEMENTATION|CYCLONEDDS_URI|AMENT_PREFIX_PATH|COLCON_PREFIX_PATH)=' <<<"${env_dump}"; then
    fail "Web 进程环境包含 ROS/RMW/CycloneDDS/AMENT/COLCON 污染变量。" "The Web process environment contains ROS/RMW/CycloneDDS/AMENT/COLCON variables."
  fi

  [[ "$(ss -ltnH 'sport = :8080' | wc -l)" -eq 1 ]] ||
    fail "8080 必须只有一个监听项；请检查 ss -ltnp 'sport = :8080'。" "Port 8080 must have exactly one listener; check ss -ltnp 'sport = :8080'."

  if [[ -x "/usr/local/sbin/${CAMERA_HELPER}" &&
        -f "/etc/sudoers.d/${CAMERA_SUDOERS%.sudoers}" ]]; then
    local helper_action=is-active
    [[ "${PRODUCT_ID}" == r1 ]] && helper_action=depth-is-active
    sudo -n "/usr/local/sbin/${CAMERA_HELPER}" "${helper_action}" >/dev/null 2>&1 || helper_rc=$?
    ((helper_rc == 0 || helper_rc == 3)) ||
      fail "相机 helper 只读状态检查失败（退出码 ${helper_rc}）。" "Read-only camera helper status check failed (exit ${helper_rc})."
  elif [[ "${install_scope}" == "system" ]]; then
    fail "系统级部署缺少相机 helper 或 sudoers。" "System deployment is missing the camera helper or sudoers rule."
  fi
}

stop_current_user_services() {
  if systemctl --user is-active --quiet "${WEB_SERVICE}" 2>/dev/null; then
    ui_line "[信息] 发现正在运行的用户级 ${WEB_SERVICE}；替换前先确认产品身份与零运动..." "[INFO] Found active user service ${WEB_SERVICE}; verifying product identity and zero motion before replacement..."
    assert_running_product
    assert_motion_stopped
    systemctl --user stop "${WEB_SERVICE}" ||
      fail "停止现有用户级 ${WEB_SERVICE} 失败。" "Failed to stop existing user service ${WEB_SERVICE}."
  fi
  if systemctl --user is-enabled --quiet "${WEB_SERVICE}" 2>/dev/null; then
    systemctl --user disable "${WEB_SERVICE}" ||
      fail "禁用现有用户级 ${WEB_SERVICE} 失败。" "Failed to disable existing user service ${WEB_SERVICE}."
  fi

  if ${NEEDS_KOKORO} && systemctl --user is-active --quiet g1-local-tts.service 2>/dev/null; then
  ...
  if ${NEEDS_KOKORO} && systemctl --user is-enabled --quiet g1-local-tts.service 2>/dev/null; then
...
[[ "$(uname -m)" == "aarch64" ]] || fail "预期 ${PRODUCT_NAME} 计算机为 aarch64，当前为 $(uname -m)。" "Expected the ${PRODUCT_NAME} computer to be aarch64, found $(uname -m)."

source /etc/os-release
[[ "${ID:-}" == "ubuntu" && "${VERSION_ID:-}" == "20.04" ]] ||
  fail "预期 ${PRODUCT_NAME} 使用 Ubuntu 20.04，当前为 ${PRETTY_NAME:-unknown}。" "Expected Ubuntu 20.04 on ${PRODUCT_NAME}, found ${PRETTY_NAME:-unknown}."
...
ip link show dev "${DDS_INTERFACE}" >/dev/null 2>&1 || fail "缺少 DDS 网卡 ${DDS_INTERFACE}。" "DDS interface ${DDS_INTERFACE} is missing."
...
if ${check_only}; then
  ...
  if [[ -f "${SYSTEM_SDK_PREFIX}/lib/cmake/unitree_sdk2/unitree_sdk2Config.cmake" ]]; then
    ui_line "[OK] SDK2 已安装在 ${SYSTEM_SDK_PREFIX}。" "[OK] SDK2 is installed in ${SYSTEM_SDK_PREFIX}."
  elif [[ -f "${USER_SDK_PREFIX}/lib/cmake/unitree_sdk2/unitree_sdk2Config.cmake" ]]; then
    ui_line "[OK] SDK2 已安装在 ${USER_SDK_PREFIX}。" "[OK] SDK2 is installed in ${USER_SDK_PREFIX}."
  ...
  if systemctl --user is-active --quiet "${WEB_SERVICE}" 2>/dev/null ||
     systemctl --user is-enabled --quiet "${WEB_SERVICE}" 2>/dev/null; then
    ui_line "[OK] 已存在持久用户级 ${WEB_SERVICE}；无交互 sudo 时自动模式可复用/更新它。" "[OK] Persistent user service ${WEB_SERVICE} is present. Auto mode may reuse/update it when interactive sudo is unavailable."
  fi
  if ${NEEDS_KOKORO} &&
     (systemctl --user is-active --quiet g1-local-tts.service 2>/dev/null ||
  ...
  if [[ -x "/usr/local/sbin/${CAMERA_HELPER}" &&
    ...
    ui_line "[信息] ${PRODUCT_NAME} 相机 helper/sudoers 尚未安装；系统级安装会补齐。" "[INFO] The ${PRODUCT_NAME} camera helper/sudoers are not installed; system install will add them."
  fi
  ui_line "[OK] ${PRODUCT_NAME} 预检查完成；没有修改软件包、文件或服务。" "[OK] ${PRODUCT_NAME} preflight completed; no packages, files, or services were changed."
  exit 0
fi
...
if [[ "${SDK_POLICY}" != "verify_existing" &&
      ! -f "${sdk_prefix}/lib/cmake/unitree_sdk2/unitree_sdk2Config.cmake" &&
...
dds_ldd="$(grep -E 'libddsc(xx)?\.so' <<<"${ldd_output}" || true)"
[[ -n "${dds_ldd}" ]] ||
  fail "Web 二进制没有链接 SDK2 CycloneDDS。" "The Web binary is not linked to SDK2 CycloneDDS."
if grep -v "${sdk_prefix}/lib/" <<<"${dds_ldd}" | grep -q .; then
  printf '%s\n' "${dds_ldd}" >&2
  fail "CycloneDDS 动态库未全部解析到所选产品 SDK 前缀 ${sdk_prefix}/lib。" "CycloneDDS libraries did not all resolve from the selected SDK prefix ${sdk_prefix}/lib."
fi
if ! ${without_realsense}; then
...
if [[ "${install_scope}" == "system" ]]; then
  ...
  sudo install -m 0755 "${project_dir}/scripts/${CAMERA_HELPER}" \
    "/usr/local/sbin/${CAMERA_HELPER}"
  sudo visudo -cf "${project_dir}/scripts/${CAMERA_SUDOERS}"
  sudo install -m 0440 "${project_dir}/scripts/${CAMERA_SUDOERS}" \
    "/etc/sudoers.d/${CAMERA_SUDOERS%.sudoers}"
  helper_rc=0
  helper_action=is-active
  [[ "${PRODUCT_ID}" == r1 ]] && helper_action=depth-is-active
  sudo -n "/usr/local/sbin/${CAMERA_HELPER}" "${helper_action}" >/dev/null 2>&1 || helper_rc=$?
  if ((helper_rc != 0 && helper_rc != 3)); then
    fail "相机 helper 的 sudoers 验证失败（退出码 ${helper_rc}）。" "Camera helper sudoers verification failed (exit ${helper_rc})."
  fi
  ui_line "[OK] 相机 helper 和免密 sudo 规则验证通过。" "[OK] Camera helper and passwordless sudo rule verified."

  if ${NEEDS_KOKORO}; then
    sudo install -m 0644 "${project_dir}/deploy/g1-local-tts.service" \
      /etc/systemd/system/g1-local-tts.service
  fi
  sudo install -m 0644 "${project_dir}/${WEB_SYSTEM_UNIT}" \
  ...
  assert_no_other_product_service
  if systemctl is-active --quiet "${WEB_SERVICE}" 2>/dev/null ||
     systemctl is-enabled --quiet "${WEB_SERVICE}" 2>/dev/null ||
     { ${NEEDS_KOKORO} && { systemctl is-active --quiet g1-local-tts.service 2>/dev/null ||
  ...
  install -m 0644 "${project_dir}/${WEB_USER_UNIT}" \
    "/home/unitree/.config/systemd/user/${WEB_SERVICE}"
  if ${NEEDS_KOKORO}; then
  ...
  systemctl --user is-active --quiet "${WEB_SERVICE}"
  if [[ ! -x "/usr/local/sbin/${CAMERA_HELPER}" ||
        ! -f "/etc/sudoers.d/${CAMERA_SUDOERS%.sudoers}" ]]; then
    ui_line "[信息] 用户级安装没有 ${PRODUCT_NAME} 特权相机 helper；相关相机能力会按运行时证据降级。" "[INFO] User installation has no privileged ${PRODUCT_NAME} camera helper; related camera capabilities will degrade based on runtime evidence."
  fi
fi
...
if [[ "${PRODUCT_ID}" == g1 ]]; then
  ui_line "g1-web-control.service 保持现有导航默认值；Web 安全确认与 10 m 目标距离限制仍然生效。" "g1-web-control.service keeps its existing navigation default; Web safety confirmation and the 10 m goal limit still apply."
else
  ui_line "r1-web-control.service 默认不声明 Mid-360，也不启用真实导航。" "r1-web-control.service does not declare Mid-360 or enable real navigation by default."
fi
