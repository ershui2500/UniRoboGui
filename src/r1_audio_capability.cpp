...
class R1AudioCapability final : public IAudioCapability {
 public:
  explicit R1AudioCapability(bool mock) : mock_(mock) {}
  ...
  bool Start(std::string& error) override {
    ...
    try {
      client_ = std::make_unique<unitree::robot::r1::AudioClient>();
      ...
      client_.reset();
      available_ = false;
      error = "unknown_r1_audio_capability_init_error";
      return false;
    }
  }
 ...
 private:
  ...
  std::unique_ptr<unitree::robot::r1::AudioClient> client_;
};

}  // namespace

std::unique_ptr<IAudioCapability> CreateR1AudioCapability(bool mock) {
  return std::make_unique<R1AudioCapability>(mock);
}
